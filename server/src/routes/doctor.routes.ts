...
router.get("/:id/slots", async (req, res) => {
  ...
  if (Number.isNaN(dayStart.getTime()))
    throw new AppError(422, "A valid booking date is required");
  const doctor = await prisma.doctor.findUnique({
    where: { id: req.params.id },
    include: { user: { select: { status: true } }, availability: true },
  });
  ...
  const availability = doctor.availability.filter(
    (slot) => slot.active && slot.dayOfWeek === dayStart.getDay(),
  );
  ...
  const booked = new Set(appointments.map((item) => item.startsAt.getTime()));
  const slots = availability.flatMap((slot) => {
    const [startHour = 0, startMinute = 0] = slot.startTime
        .split(":")
        .map(Number),
      [endHour = 0, endMinute = 0] = slot.endTime.split(":").map(Number);
    ...
    while (cursor.getTime() + 20 * 60000 <= end.getTime()) {
      values.push({
        ...
        booked: booked.has(cursor.getTime()),
      });
      cursor.setMinutes(cursor.getMinutes() + 20);
    }
    return values;
  });
  res.json({ success: true, data: { availability, slots } });
});
