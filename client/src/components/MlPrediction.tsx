...
import {Activity,BrainCircuit,CalendarDays,Check,Clock,FileHeart,Search,ShieldCheck,Sparkles,Stethoscope,X} from "lucide-react";
...
export function PredictionReportView({report}:{report:PredictionReport}){
  ...
  return <div className="ml-report">
    <article className="card ml-result-primary"><span className="pill">{lowConfidence?"LOW-CONFIDENCE SCREENING RESULT":"TRAINED MODEL RESULT"}</span><h2>{title(report.disease)}</h2><div className="ml-result-meta"><span><b>{Math.round(report.confidence*10000)/100}%</b> confidence</span><span><CalendarDays/> {date.toLocaleDateString()}</span><span><Clock/> {date.toLocaleTimeString()}</span></div>{lowConfidence&&<p className="low-confidence-note">This symptom combination overlaps with several conditions. Add a more specific fourth symptom and run the prediction again, or consult a clinician for assessment.</p>}<div className="top-predictions"><h3>Top 3 model probabilities</h3>{report.topPredictions.map((prediction,index)=><div key={prediction.disease}><span>{index+1}. {title(prediction.disease)}</span><b>{Math.round(prediction.confidence*10000)/100}%</b></div>)}</div><div className="selected-symptom-tags">{report.selectedSymptoms.map(symptom=><span key={symptom}><Check/>{title(symptom)}</span>)}</div></article>
...
export function MlPrediction(){
  ...
  const add=(symptom:string)=>{if(selected.includes(symptom))return;if(selected.length>=4){setError("You can select a maximum of 4 symptoms.");return}setSelected(items=>[...items,symptom]);setQuery("");setError("");if(selected.length===3)setSelectorOpen(false)};
  ...
  return <div className="prediction-page ml-prediction-page">
    <div className="prediction-intro"><span className="eyebrow"><Sparkles/> UHS Intelligence</span><h1>Understand what your body is telling you.</h1><p>Select three or four symptoms from the trained model dataset to generate a complete health screening report.</p><div><ShieldCheck/>Your prediction is private and saved to your patient history</div></div>
    <div className="prediction-shell">
      <section className="assessment card"><div className="step-head"><div><small>TRAINED XGBOOST MODEL</small><h2>What are you feeling?</h2></div><span>{selected.length}/4</span></div><div className="progress"><i style={{width:`${selected.length/4*100}%`}}/></div>
        ...
        {selected.length<3&&<p className="prediction-help">Please select at least 3 symptoms for a more reliable prediction.</p>}
        {error&&<p role="alert" className="form-error">{error}</p>}
        <Button onClick={predict} disabled={selected.length<3||selected.length>4||loading}>{loading?"Running trained model...":"Predict Disease"} <BrainCircuit/></Button>
      </section>
      <aside className="prediction-model-note card"><BrainCircuit/><h3>Dataset-driven screening</h3><p>Symptoms, feature order, disease labels, and recommendations come directly from the supplied model and CSV files.</p></aside>
    </div>
